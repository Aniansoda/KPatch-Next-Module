...
async function renderKpmList() {
    ...
    allKpms.forEach(module => {
        ...
        if (!item) {
            ...
            item.className = 'card module-card liquid-glass-card liquid-glass';
            item.innerHTML = `
...
function applyFilters() {
    ...
    if (visibleCount === 0) {
        ...
        emptyMsg.classList.remove('hidden');
        emptyMsg.classList.add('empty-list', 'liquid-glass');
    } else {
        emptyMsg.classList.add('hidden');
        emptyMsg.classList.remove('empty-list', 'liquid-glass');
    }
}
...
async function handleFileUpload(accept, containerId, onSelected) {
    ...
    input.onchange = async (e) => {
        ...
        loadingCard.className = 'card module-card liquid-glass-card liquid-loading';
        loadingCard.innerHTML = `
            <div class="module-card-header flex-header">
                ...
                <md-outlined-icon-button id="cancel-upload" class="liquid">
...
export function initKPMPage() {
    ...
    document.getElementById('load').classList.add('liquid');
    document.getElementById('load').onclick = () => uploadAndLoadModule();
}

export { loadModule, refreshKpmList, handleFileUpload, uploadFile }async function renderKpmList() {
    const container = document.getElementById('kpm-list');
    container.innerHTML = '';
